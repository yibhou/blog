...
在使用方括号访问数组元素时，与方括号访问对象属性一样，JavaScript会将方括号中的值转换成字符串。如果它是一个[0, 2^32^-2]之间的非负整数字符串，则这个字符串称为数组的索引。其他情况，字符串只能作为数组对象的属性名，而不是一个索引。数组的特殊之处在于，使用索引的数组会自动维护它的length属性值。实际上，数组索引也是数组对象的属性名，这意味着数组没有“越界”错误的概念。
